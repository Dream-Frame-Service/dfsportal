...
export class SecurityService {
  ...
  static async getSecuritySettings(): Promise<
    { data: SecuritySettings | null; error: string | null }
  > {
    try {
  ...
  static async addIPToWhitelist(
    ...
    try {
      const { data: settings, error: getError } = await this
        .getSecuritySettings();
  ...
  static async removeIPFromWhitelist(
    ipAddress: string,
  ): Promise<{ error: string | null }> {
    try {
      const { data: settings, error: getError } = await this
      ...
      settings.systemSecurity.ipWhitelist = settings.systemSecurity.ipWhitelist
  ...
  static async getSecurityEvents(options: {
    ...
    { data: SecurityEvent[] | null; count: number | null; error: string | null }
  > {
    try {
  ...
  static async deleteSecurityEvents(
    eventIds: string[],
  ): Promise<{ error: string | null }> {
    try {
      const { error } = await supabase
  ...
  static async logSecurityEvent(
    auditLog: Omit<SecurityAuditLog, "id">,
  ): Promise<{ error: string | null }> {
    try {
      const { error } = await supabase
        .from("audit_logs")
  ...
  static async validateSession(): Promise<
    { valid: boolean; user: any | null; error: string | null }
  > {
    try {
      ...
      if (settings) {
        const sessionTimeout = settings.accountSecurity.sessionTimeout * 60 *
  ...
  static async trackFailedLogin(
    email: string,
    ipAddress: string,
  ): Promise<{ shouldLockout: boolean; error: string | null }> {
    try {
      const { data: settings } = await this.getSecuritySettings();
      if (!settings) {
        return {
      ...
      const shouldLockout =
        attemptCount >= settings.accountSecurity.maxFailedAttempts;
      ...
      if (shouldLockout) {
        ...
        await this.logSecurityEvent({
          ...
          event_status: "success",
          action_performed:
            `Account locked due to ${attemptCount} failed login attempts`,
          risk_level: "high",
          additional_data: JSON.stringify({
